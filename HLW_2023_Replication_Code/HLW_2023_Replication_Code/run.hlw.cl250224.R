...
if ((working.dir=='') | (code.dir=='')) {
  stop("Must specify working.dir and code.dir locations in run.hlw.XXXX.R file")
}
...
cl.inflation              <- cl.data$inflation
cl.inflation.expectations <- cl.data$inflation.expectations
cl.nominal.interest.rate  <- cl.data$interest
cl.real.interest.rate     <- cl.nominal.interest.rate - cl.inflation.expectations
cl.covid.indicator        <- cl.data$covid.ind
...
cl.estimation <- run.hlw.estimation(log.output=cl.log.output,
                                    inflation=cl.inflation,
                                    real.interest.rate=cl.real.interest.rate,
                                    nominal.interest.rate=cl.nominal.interest.rate,
                                    covid.indicator=cl.covid.indicator,
...
one.sided.est.cl <- cbind(cl.estimation$out.stage3$rstar.filtered,
                          cl.estimation$out.stage3$trend.filtered,
                          cl.estimation$out.stage3$z.filtered,
                          cl.estimation$out.stage3$output.gap.filtered)

# Two-sided (smoothed) estimates
two.sided.est.cl <- cbind(cl.estimation$out.stage3$rstar.smoothed,
                          cl.estimation$out.stage3$trend.smoothed,
                          cl.estimation$out.stage3$z.smoothed,
                          cl.estimation$out.stage3$output.gap.smoothed)
...
output.cl <- format.output(country.estimation=cl.estimation,
                           one.sided.est.country=one.sided.est.cl,
                           real.rate.country=cl.real.interest.rate,
                           start=sample.start,
                           end=sample.end,
                           run.se=run.se)
