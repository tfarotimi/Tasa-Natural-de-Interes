...
if ((working.dir=='') | (code.dir=='')) {
  stop("Must specify working.dir and code.dir locations in run.hlw.co.R file")
}
...
co.data <- read.xlsx("inputData/Holston_Laubach_Williams_COL.xlsx", sheet="COL Input Data",
            na.strings = ".", colNames=TRUE, rowNames=FALSE, detectDates = TRUE)

# Filter data to match the sample period
# co.data <- subset(co.data, Date >= start.date & Date <= end.date)

co.log.output             <- co.data$gdp.log
co.inflation              <- co.data$inflation
co.inflation.expectations <- co.data$inflation.expectations
co.nominal.interest.rate  <- co.data$interest
co.real.interest.rate     <- co.nominal.interest.rate - co.inflation.expectations
co.covid.indicator        <- co.data$covid.ind
...
co.estimation <- run.hlw.estimation(log.output=co.log.output,
                                    inflation=co.inflation,
                                    real.interest.rate=co.real.interest.rate,
                                    nominal.interest.rate=co.nominal.interest.rate,
                                    covid.indicator=co.covid.indicator,
...
one.sided.est.co <- cbind(co.estimation$out.stage3$rstar.filtered,
                          co.estimation$out.stage3$trend.filtered,
                          co.estimation$out.stage3$z.filtered,
                          co.estimation$out.stage3$output.gap.filtered)

# Two-sided (smoothed) estimates
two.sided.est.co <- cbind(co.estimation$out.stage3$rstar.smoothed,
                          co.estimation$out.stage3$trend.smoothed,
                          co.estimation$out.stage3$z.smoothed,
                          co.estimation$out.stage3$output.gap.smoothed)
...
output.co <- format.output(country.estimation=co.estimation,
                           one.sided.est.country=one.sided.est.co,
                           real.rate.country=co.real.interest.rate,
                           start=sample.start,
                           end=sample.end,
                           run.se=run.se)

# Save output to CSV
write.table(output.co, 'output/output.co.csv', col.names=TRUE, quote=FALSE, row.names=FALSE, sep = ',', na = '')
print("******************** CO - estimation completed ********************")
