...
rstar.stage2 <- function(log.output,
    ...
    # print correlation between y.is and ir.is
    print("Stage 2 initial IS: Correlation between y.is and ir.is")
    print(cor(y.is, ir.is))
    ...
    if (is.na(fix.phi) & sample.end[1] >= 2020) {
        ...
        nls.is <- nls(y.is ~ phi * d + a_1 * (y.is.l1 - phi * d.l1) + a_2 * (y.is.l2 - phi * d.l2) +
            a_r * ir.is + a_0 * rep(1, t.end), start = list(phi = 0, a_1 = 0, a_2 = 0, a_r = -5, a_0 = 0))
        b.is <- coef(nls.is)
    } else {
    ...
    # plot(trend.smoothed, type = "l", col = "blue", lwd = 2, xlab = "Time", ylab = "Trend")
    potential.smoothed <- c(states$smoothed$xi.tT[1, 3:2], states$smoothed$xi.tT[, 1]) # y*_t ; not COVID-adjusted
    # plot(potential.smoothed, type = "l", col = "blue", lwd = 2, xlab = "Time", ylab = "Potential Output")
    ...
    # plot(output.gap.smoothed, type = "l", col = "blue", lwd = 2, xlab = "Time", ylab = "Output Gap")
    # plot(potential.smoothed, type="l", col="blue", lwd=2, xlab="Time", ylab="Potential Output")
    # plot(theta[param.num["phi"]]*covid.indicator[3:(t.end+4)], type="l", col="blue", lwd=2, xlab="Time", ylab="COVID Output Gap")
